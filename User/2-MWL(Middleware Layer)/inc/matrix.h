...
class Matrixf {
 public:
	...
	void getData_to_vector(Vector3 & output ){
	static float temp[_rows * _cols];
	memcpy(temp, data_, _rows * _cols * sizeof(float));
	output.x = temp [0];
	output.y = temp [1];
	output.z = temp [2];
	}
...
// cross product
Matrixf<3, 1>cross(Matrixf<3, 3> vec1, Matrixf<3, 1> vec2);
